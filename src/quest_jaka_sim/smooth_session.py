...
class SmoothQuestJakaSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.budget_exhausted_before_checks_complete = 0
        self.budget_exhausted_after_checks_nonfeasible = 0
        self.budget_exhausted_after_checks_feasible = 0
    ...
    def control_tick(self, now_ns: int) -> ArmControlTickResult:
        ...
        record.update(
            ...
            reason_before_budget_override=(
                None
                if result.reason_before_budget_override is None
                else result.reason_before_budget_override.value
            ),
            budget_exhaustion_stage=(
                None
                if result.budget_exhaustion_stage is None
                else result.budget_exhaustion_stage.value
            ),
            all_candidate_checks_completed=(
                result.all_candidate_checks_completed
            ),
            candidate_feasible_before_budget_override=(
                result.candidate_feasible_before_budget_override
            ),
            deadline_overrun_ns=result.deadline_overrun_ns,
        )
        if result.reason is FeasibilityReason.CONTROL_COMPUTE_BUDGET_EXHAUSTED:
            self.control_compute_budget_exhausted_count += 1
            if not result.all_candidate_checks_completed:
                self.budget_exhausted_before_checks_complete += 1
            elif result.candidate_feasible_before_budget_override:
                self.budget_exhausted_after_checks_feasible += 1
            else:
                self.budget_exhausted_after_checks_nonfeasible += 1
    ...
    def report(self, replay_source: str) -> dict[str, Any]:
        ...
        return {
            ...
            "budget_exhausted_before_checks_complete": (
                self.budget_exhausted_before_checks_complete
            ),
            "budget_exhausted_after_checks_nonfeasible": (
                self.budget_exhausted_after_checks_nonfeasible
            ),
            "budget_exhausted_after_checks_feasible": (
                self.budget_exhausted_after_checks_feasible
            ),
...
def _output_feasibility_attempt(
    ...
    return {
        ...
        "all_candidate_checks_completed": result.all_candidate_checks_completed,
        "candidate_feasible_before_budget_override": (
            result.candidate_feasible_before_budget_override
        ),
        "deadline_overrun_ns": result.deadline_overrun_ns,
