...
class BudgetExhaustionStage(str, Enum):
    """Stage at which an existing compute-deadline check observed exhaustion."""

    BEFORE_CANDIDATE_EVALUATION = "before_candidate_evaluation"
    DURING_IK = "during_ik"
    AFTER_IK_BEFORE_CLASSIFICATION = "after_ik_before_classification"
    AFTER_ALL_CHECKS = "after_all_checks"
...
class FeasibilityResult:
    ...
    reason_before_budget_override: FeasibilityReason | None = None
    budget_exhaustion_stage: BudgetExhaustionStage | None = None
    all_candidate_checks_completed: bool = False
    candidate_feasible_before_budget_override: bool = False
    deadline_overrun_ns: int = 0
...
class SharedJakaTargetGenerator:
    ...
    def evaluate(
        ...
        *,
        ...
        if (
            ...
            return FeasibilityResult(
                ...
                budget_exhaustion_stage=(
                    BudgetExhaustionStage.BEFORE_CANDIDATE_EVALUATION
                ),
                deadline_overrun_ns=max(
                    0, evaluate_started_ns - compute_deadline_ns
                ),
            )
        ...
        if ik_completed and compute_deadline_ns is not None:
            deadline_check_ns = time.perf_counter_ns()
            if deadline_check_ns >= compute_deadline_ns:
                self.ik.set_arm_joints_rad(ik_seed.tolist())
                timing = CandidateComputationTiming(
                    ...
                    total_ms=(
                        time.perf_counter_ns() - evaluate_started_ns
                    )
                    / 1e6,
                ...
                return FeasibilityResult(
                    ...
                    timing,
                    budget_exhaustion_stage=(
                        BudgetExhaustionStage.AFTER_IK_BEFORE_CLASSIFICATION
                    ),
                    deadline_overrun_ns=max(
                        0, deadline_check_ns - compute_deadline_ns
                    ),
                )
        if not ik_completed:
            ...
            self.ik.set_arm_joints_rad(ik_seed.tolist())
            assert compute_deadline_ns is not None
            deadline_check_ns = time.perf_counter_ns()
            timing = CandidateComputationTiming(
                seed_fk_ms=seed_fk_ns / 1e6,
                pre_ik_jacobian_ms=pre_ik_jacobian_ns / 1e6,
        ...
        if compute_deadline_ns is None:
            reason_before_budget_override = None
            budget_exhaustion_stage = None
            candidate_feasible_before_budget_override = False
            ...
            if deadline_check_ns >= compute_deadline_ns:
                ...
                reason_before_budget_override = reason
                candidate_feasible_before_budget_override = (
                    reason_before_budget_override is FeasibilityReason.ACCEPTED
                )
        ...
        return FeasibilityResult(
            ...
            metrics,
            timing,
            reason_before_budget_override=reason_before_budget_override,
            budget_exhaustion_stage=budget_exhaustion_stage,
            all_candidate_checks_completed=True,
            candidate_feasible_before_budget_override=(
                candidate_feasible_before_budget_override
            ),
            deadline_overrun_ns=deadline_overrun_ns,
        )
