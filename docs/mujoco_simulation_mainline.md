...
Check that the cyan active collision proxies sit near the real RH56 finger pads, especially in `real_pinch_v4`. If the active proxies are visibly on the phalanx root instead of the pad, grasp metrics are not meaningful yet.
...
重点看 `real_pinch_v4`：cyan active collision proxies 应该落在真实 RH56 指腹附近。如果它们明显落在指节根部而不是指腹，当前 grasp metric 就还没有意义。
