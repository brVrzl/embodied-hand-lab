...
the combined entry has multiple short physical run records, but no run is a
complete, long-duration, all-gates PASS.
...
heartbeats while index is released. This correction is offline-tested. Later
short combined runs exercised the entry and exposed additional retained timing,
Quest-liveness, and producer-liveness stop paths; none establishes an overall
combined PASS.
...
RH56 Quest hand-only 已完成一次 60 秒真机运行。联合入口已有多次短时真机运行记录，但
尚无完整、长期、所有 gate 均 PASS 的验证。
...
已完成离线测试；后续短时联合运行又覆盖到保留的 timing、Quest-liveness 和 producer-
liveness 停止路径，但都不能作为联合整体 PASS。
