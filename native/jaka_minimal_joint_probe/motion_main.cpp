...
struct Options{bool vendor=false,physical=false,estop=false,workspace=false,no_person=false,cables=false,direction=false,ready=false,five=false;std::string ip,edg,ack,output,csv;int tool=0,user=0;double fake_tracking_offset=0,fake_tracking_growth=0,fake_non_target_offset=0;jaka_zero::FakeOptions fake{};};
...
Options parse(int n,char**v){Options o;for(int i=1;i<n;++i){std::string a=v[i];
 if(a=="--backend"){auto x=val(i,n,v);o.vendor=x=="vendor";if(x!="vendor"&&x!="fake")throw std::runtime_error("bad backend");}
...
int execute(const Options&o){if(!o.vendor&&!o.physical&&o.output.empty()){std::cout<<"{\"mode\":\"fake_requires_output\",\"commands_issued\":0}\n";return 0;}const double d=o.five?kD5:kD;stop_requested=false;signal(SIGINT,signal_handler);signal(SIGTERM,signal_handler);
