...
The PC-direct RH56 path has physical evidence for actual-device read-only
feedback, bounded commands, and a 60 second Quest hand-only run on 2026-07-29
without timeout, checksum, protocol, or hand fault. The combined JAKA + RH56
wrapper has also been exercised in multiple short physical runs, but none is a
complete, long-duration, all-gates PASS. It retains separate exact approvals.
...
PC-direct RH56 已完成真实设备 read-only feedback、受限 command，并于 2026-07-29 完成
一次 60 秒 Quest hand-only 真机运行，无 timeout、checksum、protocol 或 hand fault。
JAKA + RH56 联合 wrapper 也已有多次短时真机运行记录，但尚无完整、长期、所有 gate 均
PASS 的验证；联合入口继续要求两套精确授权。
