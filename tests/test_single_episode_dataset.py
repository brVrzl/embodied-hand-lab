...
def test_trigger_bounds_exactly_one_episode_and_excludes_idle(
    ...
    with h5py.File(act_path, "r") as dataset:
        assert dataset["action"].shape == (2, 12)
        assert dataset["observations/qpos"].shape == (2, 12)
        assert dataset["observations/qvel"].shape == (2, 6)
        assert dataset["observations/images/workspace"].shape == (2, 8, 10, 3)
